...
def check_availability(course, crn, term="Winter 2018", dept="COMP"):
    ...
    for row in rows:
        if crn in row.text:
            ...
            rem_wait = row.find_elements_by_tag_name("td")[15].text
            status = row.find_elements_by_tag_name("td")[19].text
            info_dict["spots"] = rem
            info_dict["wait_spots"] = rem_wait

            if(int(rem_wait) > 0):
                driver.find_element_by_xpath("/html/body/div[3]/form/table/tbody/tr[3]/td[1]/input[1]").click()
                driver.find_element_by_xpath("/html/body/div[3]/form/input[7]").click()
